...
export function HEProjectDetail() {
  ...
  return (
    <div className="p-5" style={{ background: '#F7F8FA', minHeight: '100vh' }}>
      {/* Header */}
      <div className="flex items-start gap-4 mb-5">
        <button onClick={() => navigate('/tob/he/projects')} className="mt-1 transition-colors hover:opacity-70" style={{ color: '#86909C' }}>
          <ArrowLeft size={18} />
        </button>
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-2">
            <h1 className="text-base" style={{ fontWeight: 700, color: '#1D2129' }}>{project.name}</h1>
          ...
          <div className="flex items-center gap-4 text-sm flex-wrap" style={{ color: '#86909C' }}>
            ...
            <span className="flex items-center gap-1"><Users size={13} />{project.memberCount} 人团队</span>
          </div>
          {project.paradigm && <p className="text-sm mt-1.5" style={{ color: '#86909C' }}>实验范式：{project.paradigm}</p>}
        </div>
